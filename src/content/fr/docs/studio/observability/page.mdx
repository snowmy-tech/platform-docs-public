...
:::note
L’utilisation pendant la préversion privée est incluse dans votre plan Enterprise. La tarification peut évoluer au moment de la disponibilité générale.
:::
...
Les traces sont collectées avec [OpenTelemetry](https://opentelemetry.io/) et sont prises en charge dans les produits Mistral : le SDK Mistral (Python et TypeScript), Workflows, Vibe Code CLI, Vibe Code Web et Vibe Work. Toute application instrumentée avec OpenTelemetry peut également envoyer des traces directement.

Une fois les traces collectées, l'[Explorateur de traces](https://console.snowmy.app/observability/traces) dans Studio vous permet de rechercher et filtrer toutes les requêtes, d'inspecter les exécutions individuelles de bout en bout et de déboguer les échecs au niveau du span.
