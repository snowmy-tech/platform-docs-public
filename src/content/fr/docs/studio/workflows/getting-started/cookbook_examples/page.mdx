...
1. Un [compte Mistral](https://console.snowmy.app/).
2. Python 3.12 installé.
3. [uvx](https://docs.astral.sh/uv/guides/tools/) disponible dans votre shell.
4. Une [clé API Mistral](https://console.snowmy.app/home?profile_dialog=api-keys) pour l'espace de travail sur lequel vous souhaitez lancer vos workflows.

Vous pouvez consulter la chronologie d'exécution ou réaliser une étape de validation humaine dans [Studio](https://console.snowmy.app/build/workflows/executions).
...
Vous pouvez aussi démarrer `cargo-release-compliance` dans [Studio](https://console.snowmy.app/build/workflows?workflowSearch=cargo-release-compliance) avec cet input&nbsp;:
...
Dans [Studio](https://console.snowmy.app/build/workflows/executions), inspectez le sous-workflow de vérification des marchandises dangereuses, puis repérez l'événement `wait_for_input` sur le parcours anomalie. Approuvez ou bloquez l'expédition, et vérifiez que le workflow parent reprend à l'étape de validation plutôt que de réexécuter les étapes précédentes.
...
Ou démarrer le workflow dans [Studio](https://console.snowmy.app/build/workflows?workflowSearch=insurance-claims-triage) avec ces données&nbsp;:
...
Dans [Studio](https://console.snowmy.app/build/workflows/executions), vérifiez que les activités `analyze_photo` démarrent en parallèle, puis inspectez les étapes de cohérence, le scoring de fraude et le rapport final typé. L'aiguillage est défini par le code du workflow, pas par un appel à un modèle séparé.
...
Vous pouvez aussi démarrer `code-modernization` dans [Studio](https://console.snowmy.app/build/workflows?workflowSearch=code-modernization) avec cet input&nbsp;:
...
Dans [Studio](https://console.snowmy.app/build/workflows/executions), vérifiez que chaque fichier source apparaît comme sa propre exécution fille. Après la fin des sous-workflows, repérez la pause `wait_for_input`, approuvez ou refusez le jeu de modifications, puis assurez-vous que le workflow ne crée la PR qu'après validation.
...
1. Créez un **Connecteur Linear** dans [Studio › Contexte › Connecteurs](https://console.snowmy.app/build/connectors). Nommez-le `linear` pour qu’il corresponde au code du workflow.
...
Pour plus de détails, consultez [déploiements renforcés](/studio/workflows/managing-workflows-in-production/hardened_deployments) et [connecteurs](/studio/workflows/building-workflows/connectors).
...
Quand aucune entrée n’est fournie, le workflow marque une pause et invite à entrer l’équipe et le Projet dans [Studio](https://console.snowmy.app/build/workflows/linear-weekly-summary). Vous pouvez également fournir une entrée directement :
...
Ou lancez `linear-weekly-summary` dans [Studio](https://console.snowmy.app/build/workflows?workflowSearch=linear-weekly-summary) avec cette entrée :
...
Pour comprendre l'architecture de ces exemples, poursuivez avec [Concepts fondamentaux - Workflows](/studio/workflows/getting-started/core_concepts/workflows) et [Construire des workflows](/studio/workflows/building-workflows/workflows).
