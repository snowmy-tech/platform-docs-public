...
Les logs d’exécution sont également disponibles dans Studio. Ouvrez <AppLink href="https://console.snowmy.app/build/workflows/executions" app="studio" path={["Workflows", "Executions"]} /> et sélectionnez une exécution pour consulter ses logs sur la page correspondante.
...
Par défaut, toutes les traces sont exportées (le worker échantillonne à un taux de 100 %). Pour définir l’ID de trace ou contrôler l’échantillonnage d’une exécution, transmettez un `traceparent` dans le Corps de texte de votre requête d’exécution. Le worker utilise un échantillonneur basé sur le parent ; il suit donc votre décision d’échantillonnage.
...
Nos SDK le font pour vous. Ils se chargent d'envoyer le traceparent du span actuel à chaque Appel Exécuter.

**L’utilisation de `traceparent` comme en-tête HTTP est dépréciée.** Cette méthode fonctionne encore pour l’instant, mais les en-têtes peuvent être réécrits en transit ; la valeur que vous envoyez peut donc être différente de celle que nous recevons. Utilisez plutôt le paramètre Corps de texte.
