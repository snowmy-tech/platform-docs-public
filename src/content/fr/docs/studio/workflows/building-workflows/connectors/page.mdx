...
Vous devez également avoir enregistré au moins un [Connecteur](/studio/connectors) pour votre espace de travail. Créez-en un via <a href="https://console.snowmy.app/build/connectors" target="_blank" rel="noopener">Studio &rsaquo; Contexte &rsaquo; Connecteurs</a>, ou via l'[API Connecteurs](/studio/connectors/management).
...
1. Ouvrez <a href="https://console.snowmy.app/build/connectors" target="_blank" rel="noopener">Studio &rsaquo; Contexte &rsaquo; Connecteurs</a> puis sélectionnez un connecteur.
...
:::info
L'API `client.beta.connectors` est en **Public Preview**. Consultez le [cookbook d'authentification multiple](https://github.com/mistralai/connector-sdk-demo/blob/main/cookbooks/06-multiple-authentication.md) pour un exemple complet Bearer + OAuth2.
:::
...
| `allow_mcp_ui` | `False` | Autorise les outils MCP dotés d'une app visible sur ce connecteur à afficher leur app `ui://` comme side app lorsqu'ils sont appelés avec `ToolCallClient.call_tool()` (voir [Afficher des MCP Apps depuis des outils de connecteur](#mcp-apps)). |
| `run_as` | `"auto"` | Identité Mistral utilisée pour accéder au Connecteur. `"auto"` suit l’option `on_behalf_of` du Workflow (utilisateur déclencheur en OBO, worker sinon) ; `"deployment"` utilise toujours l’identité de service du worker. Voir [Choisir l’identité du Connecteur](#run-as). |
...
<SectionTab as="h2" sectionId="run-as">Choisir l’identité du Connecteur (run_as)</SectionTab>
...
Ouvrez <a href="https://console.snowmy.app/build/workflows" target="_blank" rel="noopener">Studio &rsaquo; Workflows</a>, sélectionnez votre workflow puis cliquez sur **Démarrer le workflow**.
...
- Pour ajouter ou mettre à jour des identifiants par connecteur avant de lancer un workflow, rendez-vous dans <a href="https://console.snowmy.app/build/connectors" target="_blank" rel="noopener">Studio &rsaquo; Contexte &rsaquo; Connecteurs</a> puis ouvrez l’onglet **Identifiants**.
...
<SectionTab as="h2" sectionId="mcp-apps">Afficher des MCP Apps depuis des outils de connecteur</SectionTab>
