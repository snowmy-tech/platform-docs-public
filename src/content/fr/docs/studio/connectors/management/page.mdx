...
1. **Testez** le serveur MCP dans le [Débogueur de Connecteurs](/studio/connectors/debugger) si vous souhaitez valider la connectivité au préalable.
...
5. **Utilisez** le Connecteur dans des [conversations](/studio/connectors/conversations) ou via des [appels directs d’outils](/studio/connectors/tool_calling).
...
Lorsqu’un Connecteur est créé pour la première fois, un appel d’authentification est requis pour se connecter, même sans identifiants.

Pour se connecter lorsque les identifiants ne sont pas nécessaires, passez des identifiants vides :
...
:::caution
La transmission de tokens de manière programmatique n’est pas prise en charge. Utilisez [Studio](https://console.snowmy.app/build/connectors) pour authentifier les Connecteurs à la place.
:::
...
Listez les outils exposés par un Connecteur. Cela permet de vérifier les noms des outils disponibles avant de les utiliser dans des [conversations](/studio/connectors/conversations) ou de [les appeler directement](/studio/connectors/tool_calling).
