...
1. **Test** the MCP server in the [Connectors Debugger](/studio/connectors/debugger) if you want to validate connectivity first.
...
5. **Use** the Connector in [conversations](/studio/connectors/conversations) or via [direct tool calls](/studio/connectors/tool_calling).
...
:::caution
Passing tokens programmatically is not supported. Use [Studio](https://console.snowmy.app/build/connectors) to authenticate Connectors instead.
:::
...
List the tools a Connector exposes. This helps verify available tool names before using them in [conversations](/studio/connectors/conversations) or [calling them directly](/studio/connectors/tool_calling).
