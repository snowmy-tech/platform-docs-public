...
The returned `execution` object contains the `execution_id` and the initial `status` (typically `RUNNING`). You can also trigger executions from [Studio](https://console.snowmy.app/), from [Vibe Work](https://chat.mistral.ai/), or via a schedule.
...
For the full event log, use the history endpoint (`GET /v1/workflows/executions/{execution_id}/history`). For live progress as the workflow runs, see [Streaming](/studio/workflows/building-workflows/streaming).
