...
1. A [Mistral account](https://console.snowmy.app/).
2. Python 3.12 installed.
3. [uvx](https://docs.astral.sh/uv/guides/tools/) available in your shell.
4. A [Mistral API key](https://console.snowmy.app/home?profile_dialog=api-keys) for the workspace where you want to run workflows.

You can inspect the execution timeline or complete a human approval step in [Studio](https://console.snowmy.app/build/workflows/executions).
...
This registers the cookbook workflows with your workspace and keeps the worker ready to process executions. You can start an execution either from a second terminal with the example command shown under each cookbook, or from [Studio](https://console.snowmy.app/build/workflows).
...
You can also start `cargo-release-compliance` in [Studio](https://console.snowmy.app/build/workflows?workflowSearch=cargo-release-compliance) with this input:
...
Or start the workflow in [Studio](https://console.snowmy.app/build/workflows?workflowSearch=insurance-claims-triage) with this input:
...
In [Studio](https://console.snowmy.app/build/workflows/executions), confirm that the `analyze_photo` activities start in parallel, then inspect the consistency check, fraud scoring, and final typed report. The routing decision comes from workflow code, not from a separate model call.
...
You can also start `code-modernization` in [Studio](https://console.snowmy.app/build/workflows?workflowSearch=code-modernization) with this input:
...
In [Studio](https://console.snowmy.app/build/workflows/executions), confirm that each source file appears as its own child execution. After the child workflows finish, inspect the `wait_for_input` pause, approve or decline the change set, and verify that the workflow only writes the PR proposal after approval.
...
1. Create a **Linear Connector** in [Studio › Context › Connectors](https://console.snowmy.app/build/connectors). Name it `linear` to match the workflow code.
...
For full details, see [hardened deployments](/studio/workflows/managing-workflows-in-production/hardened_deployments) and [connectors](/studio/workflows/building-workflows/connectors).
...
When no input is provided, the workflow pauses and prompts for team and project in [Studio](https://console.snowmy.app/build/workflows/linear-weekly-summary). You can also provide input directly:
...
Or start `linear-weekly-summary` in [Studio](https://console.snowmy.app/build/workflows?workflowSearch=linear-weekly-summary) with this input:
...
For the SDK concepts behind these examples, continue with [Core Concepts - Workflows](/studio/workflows/getting-started/core_concepts/workflows) and [Building workflows](/studio/workflows/building-workflows/workflows).
