...
Use [Connectors](/studio/connectors) inside a workflow to call external services, such as GitHub, Notion, Slack, and Outlook, without managing credentials yourself. The workflow declares which Connectors it needs. The Mistral platform resolves credentials at runtime and triggers OAuth flows on demand.

A workflow resolves Connector credentials from the identity it runs under:

- **[On-behalf-of (OBO) workflows](/studio/workflows/building-workflows/on_behalf_of)** use the **triggering user's** credentials. OBO requires a [hardened deployment](/studio/workflows/managing-workflows-in-production/hardened_deployments). For setup steps, see [Register an OBO workflow](/studio/workflows/managing-workflows-in-production/hardened_deployments#registering-an-obo-workflow).
...
You also need at least one [Connector](/studio/connectors) registered for your Workspace. Create one from <AppLink href="https://console.snowmy.app/build/connectors" app="studio" path={["Context", "Connectors"]} />, or with the [Connectors API](/studio/connectors/management).
...
1. Open <AppLink href="https://console.snowmy.app/build/connectors" app="studio" path={["Context", "Connectors"]} /> and select a Connector.
...
| `run_as` | `"auto"` | Mistral identity used to access the Connector. `"auto"` follows the Workflow's `on_behalf_of` flag (triggering user under OBO, worker otherwise); `"deployment"` always uses the worker's service identity. See [Choose the Connector identity](#run-as). |
...
<SectionTab as="h2" sectionId="run-as">Choose the Connector identity (run_as)</SectionTab>

The `run_as=` argument on `connector()` lets you select which Mistral identity is used to access a specific Connector. Depending on your use case, you may want to leverage the executor's data through their Connectors, or expose a pre-configured Connector independent of the executor.

Each Connector runs its credential preflight (resolve, list credentials, OAuth, tool validation) and its tool calls under one identity. `run_as=` sets it per Connector:

| `run_as` | Runs as |
|---|---|
| `"auto"` (default) | Follows the Workflow's `on_behalf_of` flag: the **triggering user's** credentials when the Workflow runs on behalf of a user, the **worker's** credentials otherwise. |
| `"deployment"` | Always the **worker's** (deployment's) service identity, regardless of how the Workflow runs. |
...
Because `run_as` is set per Connector, a single Workflow can mix identities. For example, read the triggering user's GitHub with `run_as="auto"` while posting to a shared Slack with `run_as="deployment"`. To have an `auto` Connector act as the triggering user, define the Workflow with `on_behalf_of=True`.

:::warning
- **`deployment` cannot run interactive OAuth.** A `deployment` Connector runs as the worker's service identity, which has no interactive user. If it has no usable credentials and requires OAuth2, the preflight fails fast instead of emitting an auth URL, add the worker's credentials in Studio beforehand.
- **Durable Agents need a single `run_as`.** All Connectors attached to one `Agent` must share the same `run_as`, because the agent's conversation runs as one identity; mixed values are rejected. Use `ToolCallClient` for per-Connector identity.
:::
...
Open <AppLink href="https://console.snowmy.app/build/workflows" app="studio">Workflows</AppLink>, pick your workflow, and click `Start workflow`.
...
- To add or update credentials per Connector before starting a workflow, go to <AppLink href="https://console.snowmy.app/build/connectors" app="studio" path={["Context", "Connectors"]} /> and open the `Credentials` tab.
...
Pass a Connector slot directly to a [Durable Agent](/studio/workflows/building-workflows/durable_agents) to let the agent call Connector tools autonomously during its conversation loop. Keep `@uses_connectors` on the workflow so the auth interceptor still runs:
