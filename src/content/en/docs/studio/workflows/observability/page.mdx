...
Execution logs are also available in Studio. Open <AppLink href="https://console.snowmy.app/build/workflows/executions" app="studio" path={["Workflows", "Executions"]} /> and select an execution to inspect its logs on the execution page.
...
By default, all traces are exported (the worker samples at a 100% rate). To set the trace ID or control sampling for an execution, pass a `traceparent` in the body of your execute request. The worker uses a parent-based sampler, so it follows your sampling decision.
...
Our SDKs do this for you. They send the traceparent of the current span on every execute call.

**Passing `traceparent` as an HTTP header is deprecated.** It still works for now, but headers can be rewritten in transit, so the value you send may not be the one we receive. Use the body parameter instead.
